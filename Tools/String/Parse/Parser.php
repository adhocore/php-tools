...
class Parser {
					...
					if ( $value = $this->getChar($tokenValue.$lookaheadValue) ) {
						...
						if ( $try = $this->getChar($tokenValue) ) {
							...
							if ( in_array($tokenValue, array('Gn', 'Gy')) ) {
								...
								if ($lookaheadType === Lexer::T_VOWEL) $this->lexer->moveLexer();
								else $try = true;
							}
							else {
	...
	private function getNumber( $number ){		
		...
		for ( $i=0; $i < strlen($number); $i++ ) {
			...
			if ( ! isset($this->numMap[$digit]) ) $return .= $digit;
			else {
				$return .= ( $this->html ) 
						? '&#' . $this->numMap[$digit] . ';'  
						: $this->getUTF8($this->numMap[$digit]);	
            }
		}		
		return $return;	
	}	
